...
# modified by Yukun to deal with new TF version following https://github.com/bioinf-jku/TTUR/issues/6 
def _get_inception_layer(sess):
    ...
    for op_idx, op in enumerate(ops):
        for o in op.outputs:
            shape = o.get_shape()
            #if shape._dims is not None: # original?
            if shape._dims != [] and (shape._dims is not None):
              ...
              for j, s in enumerate(shape):
                if s == 1 and j == 0:
                  ...
                  new_shape.append(s)
              #o._shape = tf.TensorShape(new_shape)# original?
...
def check_or_download_inception(inception_path):
    ...
    print('checking whether {} exists...'.format(model_file))
    if not model_file.exists():
        print("Downloading Inception model")
        # from urllib import request
        ## changed by Yukun due to import error
        import urllib.request as request 
        #import urllib2 as request
        import tarfile
        # fn, _ = request.urlretrieve(INCEPTION_URL)
        fn = request.urlopen(INCEPTION_URL)
...
def _handle_path(path, sess):
    if path.endswith('.npz'):
        ...
        print('Reading data from {}'.format(path))# Yukun
